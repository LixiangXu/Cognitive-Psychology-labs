/************************************** 
 * Decision-Making and Personal Preferences *
 **************************************/
...
let expName = 'Decision-Making and Personal Preferences';  // from the Builder filename that created this script
let expInfo = {
...
async function experimentInit() {
  ...
  trial_outcomes = [];
  choice_page_duration = 2.0;
  var choice_page_duration = 2.0; // Default fallback duration
...
function excitement_question1RoutineBegin(snapshot) {
  return async function () {
    ...
    excitement_question1MaxDuration = 4
    // keep track of which components have finished
...
function main_fractalRoutineEnd(snapshot) {
    return async function () {
      ...
      if ((choice_resp.keys === "left")) {
          ...
          choice_page_duration = Math.max(0, 6.0 - rt);
...
function outcome_pageRoutineBegin(snapshot) {
    return async function () {
      ...
      // update component parameters for each repeat
      outcome_headline.setText('');
      outcome.setText(outcome_display);
      // Run 'Begin Routine' code from outcome_page_code
      if (choice_timeout) {
          outcome_headline.setText("No response detected. This trial does not count.");
      } else {
          outcome_headline.setText("The chance you will write about your personal experience:");
      }
...
function excitement_question2RoutineBegin(snapshot) {
    return async function () {
      ...
      excitement_question2MaxDuration = 4
      // keep track of which components have finished
